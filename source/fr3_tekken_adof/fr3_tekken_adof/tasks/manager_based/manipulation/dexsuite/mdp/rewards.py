...
def contacts(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    thumb_contact_sensor: ContactSensor = env.scene.sensors["Thumb_Distal_Phalanx_object_s"]
    index_contact_sensor: ContactSensor = env.scene.sensors["Index_Distal_Phalanx_object_s"]
    middle_contact_sensor: ContactSensor = env.scene.sensors["Middle_Distal_Phalanx_object_s"]
    ring_contact_sensor: ContactSensor = env.scene.sensors["Ring_Distal_Phalanx_object_s"]
    pinky_contact_sensor: ContactSensor = env.scene.sensors["Pinky_Distal_Phalanx_object_s"]
...
def lift_height(
    ...
    """Reward lifting the object above a given height, but only when grasp is good."""

    object: RigidObject = env.scene[object_cfg.name]
    z = object.data.root_pos_w[:, 2]

    # how far above the lift_height we are (0..1).
    height_term = torch.clamp((z - lift_height) / lift_height, min=0.0, max=1.0)

    # require thumb + at least one finger in contact
    grasp_mask = contacts(env, threshold=1.0).float()

    return height_term * grasp_mask
